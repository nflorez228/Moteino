...
#define REPORT_CYCLES  1 // read and report battery voltage every this many sleep cycles (ex 30cycles * 8sec sleep = 240sec/4min). For 450 cyclesyou would get ~1 hour intervals
...
void loop()
{
    ...
    while(ntermino)
    {
        ...
        if (currPeriod != lastPeriod)
        {
            ...
            if (sendWithRtry("1:254-iD:TEM LOGIN"))
                Serial.print(" OOK!");
    ...
    if (tempReportCycles == REPORT_CYCLES)
    {
        digitalWrite(SENSEN_PIN,HIGH);
        float h = dht.readHumidity();
        float t = dht.readTemperature();
        float f = dht.readTemperature(true);
        float hfi = dht.computeHeatIndex(f, h);
        float hci = 0;
        if (isnan(h) || isnan(t) || isnan(f) || isnan(hfi))
        {
        ...
        else
        {
            ...
            digitalWrite(LED, HIGH);
            sprintf(sendBuf, "1:4-iD:TEM %s",tosend);
    ...
    if (batteryReportCycles == BATT_CYCLES)
    {
        sprintf(sendBuf, "V:%s", BATstr);
        sendLen = strlen(sendBuf);
