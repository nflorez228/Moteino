...
void loop()
{
    ...
    while(ntermino)
    {
        ...
        if (currPeriod != lastPeriod)
        {
            ...
            if (radio.sendWithRetry(GATEWAYID, "(XXX)iD:MOV LOGIN", 17))
            Serial.print(" ok!");
    ...
    for(int i=0;i<2;i++)
    {
        ...
        if (motionDetected)
        {
            ...
            {
                sprintf(sendBuf, "(%d)iD:MOV M:0 V:%s",EEPROM.read(1), BATstr);
            }
            else
            {
                sprintf(sendBuf, "(%d)iD:MOV M:1 V:%s",EEPROM.read(1), BATstr);
            }
            DEBUG(sendBuf);
