...
void checkSwitch2()
{
  if (cycleSWCount == SWIT_CYCLES) //only read battery every BATT_CYCLES sleep cycles
    {
      if(digitalRead(MOTION2_PIN)==LOW)
      {
        cycleSWCount=0;
        motionDetected=true;
    ...
    while(ntermino)
    {
        ...
        if (radio.receiveDone())
        {
            ...
            if(textoFull.toInt()>1 && textoFull.toInt()<250 )
            {
                ...
                motionDetected=true;
                ntermino=false;
                sprintf(sendBuf, "(%d)iD:OUD O1:OFF O2:OFF", EEPROM.read(1));
        ...
        {
          Blinkr(RELAY_PIN,300,1);
            lastPeriod=currPeriod;
            if (radio.sendWithRetry(GATEWAYID, "(XXX)iD:OUD LOGIN", 17))
                Serial.print(" ok!");
    ...
    for(int i=0;i<1;i++)
    {
        ...
        if (motionDetected)
        {
            ...
            if(!relay1State)
            {
                ...
                {
                    sprintf(sendBuf, "(%d)iD:OUD O1:OFF O2:ON", EEPROM.read(1));
                    digitalWrite(RELAY_PIN, LOW);
            ...
            else if(relay1State)
            {
                ...
                if(!relay2State)
                {
                    sprintf(sendBuf, "(%d)iD:OUD O1:ON O2:OFF", EEPROM.read(1));
                ...
                {
                    sprintf(sendBuf, "(%d)iD:OUD O1:ON O2:ON", EEPROM.read(1));
                  digitalWrite(RELAY_PIN, HIGH);
                  digitalWrite(RELAY2_PIN, HIGH);
                  Serial.println("ONM2");
                }
        ...
        if (inputstr != "")
        {
            ...
            {
                Serial.println("OFF");
                sprintf(sendBuf, "(%d)iD:OUD O1:OFF", EEPROM.read(1));
                sendLen = strlen(sendBuf);
                radio.sendWithRetry(GATEWAYID, sendBuf, sendLen);
                //radio.sendWithRetry(GATEWAYID, "iD:OUD O1:OFF", 13);
                digitalWrite(RELAY_PIN, LOW);
            ...
            {
                Serial.println("ON");
                sprintf(sendBuf, "(%d)iD:OUD O1:ON", EEPROM.read(1));
                sendLen = strlen(sendBuf);
                radio.sendWithRetry(GATEWAYID, sendBuf, sendLen);
                //radio.sendWithRetry(GATEWAYID, "iD:OUD O1:ON", 12);
                digitalWrite(RELAY_PIN, HIGH);
            ...
            {
                Serial.println("OFF2");
                sprintf(sendBuf, "(%d)iD:OUD O2:OFF", EEPROM.read(1));
                sendLen = strlen(sendBuf);
                radio.sendWithRetry(GATEWAYID, sendBuf, sendLen);
                //radio.sendWithRetry(GATEWAYID, "iD:OUD O2:OFF", 13);
                digitalWrite(RELAY2_PIN, LOW);
            ...
            {
                Serial.println("ON2");
                sprintf(sendBuf, "(%d)iD:OUD O2:ON", EEPROM.read(1));
                sendLen = strlen(sendBuf);
                radio.sendWithRetry(GATEWAYID, sendBuf, sendLen);
