...
#define BATT_MONITOR  A0  // Sense VBAT_COND signal (when powered externally should read ~3.25v/3.3v (1000-1023), when external power is cutoff it should start reading around 2.85v/3.3v * 1023 ~= 883 (ratio given by 10k+4.7K divider from VBAT_COND = 1.47 multiplier)
...
void loop()
{
    ...
    while(ntermino)
    {
        ...
        if (currPeriod != lastPeriod)
        {
            ...
            if (sendWithRtry("1:254-iD:H2O LOGIN"))
                Serial.print(" OOK!");
    ...
    if (sensorValue>6)
    {
        while(sensorValue>6)
        {
            digitalWrite(LED, HIGH);
            delay(500);
            sprintf(sendBuf, "iD:H2O W:1 V:%s", BATstr);
            DEBUG(sendBuf);
        ...
        sprintf(sendBuf, "iD:H2O W:0 V:%s", BATstr);
        DEBUG(sendBuf);
        sendLen = strlen(sendBuf);
        mySerial.println(sendBuf);
        /*if (radio.sendWithRetry(GATEWAYID, sendBuf, sendLen))
